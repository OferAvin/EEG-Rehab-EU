...
parser = argparse.ArgumentParser(
    description='Subject-adaptative classification with SHU Data')
...
dat_list=sio.loadmat('data_list.mat')['list']
...
for day in range(1,6):
    ...
    for fold, subj in enumerate(subjs):
        ...
        allf=0
        if day >1:
            for dy in range(day-1):
                allf+=dat_list[fold*5+dy,2]
                # sessionf=dat_list[fold*5+dy,2]
                # trf=allf-int(sessionf*(val_rate+test_rate))
                # valf=allf-int(sessionf*(test_rate))



        session_len=dat_list[fold*5+day-1,2]

        X, Y = get_data(subj)
        # if day>1:
        #     staticData =X[:allf]
        #     staticLabels=Y[:allf].astype(np.int64)
        # else:
        staticData=np.empty((0,X.shape[1],X.shape[2]))
        staticLabels=[]


        cvdata=X[allf:allf+session_len]
        cvlabels=Y[allf:allf+session_len]

        cv = StratifiedKFold(10, shuffle=False)  # k折验证  StratifiedKFold      ShuffleSplit
        cv_split = cv.split(cvdata, cvlabels)

        cv_acc=[]
        cvs=1
        for cvi,cvj in cv_split:
            trdata,trlabels=cvdata[cvi],cvlabels[cvi]
            X_test,Y_test=cvdata[cvj],cvlabels[cvj]

            trXlen=int(len(trdata)*0.9)     # 0.9 is the training set ratio
            valXlen=int(len(trdata)*1)    # 1 is the hole trianing set including training set and validation set.



            X_train,Y_train=trdata[:trXlen],trlabels[:trXlen].astype(np.int64)
            X_val,Y_val=trdata[trXlen:valXlen],trlabels[trXlen:valXlen].astype(np.int64)
            ...
            model.epochs_df.to_csv(pjoin(outpath, 'day='+str(day),'epochs' + suffix +'_'+str(cvs)+ '.csv'))
            test_loss = model.evaluate(X_test, Y_test)
            with open(pjoin(outpath, 'day='+str(day), 'test' + suffix +'_'+str(cvs)+ '.json'), 'w') as f:
                json.dump(test_loss, f)
            cv_acc.append(1-test_loss['misclass'])
            cvs+=1
        test_acc.append([subj,cv_acc])
