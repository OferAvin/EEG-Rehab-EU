...
def parseSHUFile(dataPath, epochWindow = [0,4], chans = list(range(30))):
    fs=250
    ch_names = ["Fp1", "Fp2", "Fz", "F3", "F4", "F7", "F8", "FC1", "FC2", "FC5",
                "FC6", "Cz", "C3", "C4", "T3", "T4", "A1", "A2", "CP1", "CP2",
                "CP5", "CP6", "Pz", "P3", "P4", "T5", "T6", "PO3", "PO4", "Oz",
                "O1"]
    tr_da=loadmat(dataPath)
    x_train=tr_da['data'].transpose(1,2,0)
    y_train=np.ravel(tr_da['labels']).astype(np.int64)-1

    # y_train=(y_train/2).astype(np.int64)
...
def parseSHUDataset(datasetPath, savePath, epochWindow=[0, 4], chans=list(range(30))):
    ...
    for iSubs, subs in enumerate(subAll):
        for iSub, sub in enumerate(subs):
            data = parseSHUFile(os.path.join(datasetPath, sub + '.mat'), epochWindow=epochWindow, chans=chans)
...
def parseBci42aFile(dataPath, labelPath, epochWindow = [0,4], chans = list(range(22))):
    ...
    '''
    # eventCode = [2]  # start of the trial at t=0. # Fixed 2020.12.15 11:35
    fs = 250
    offset = 2
