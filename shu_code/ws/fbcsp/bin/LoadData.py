...
ch_names_list = ["Fp1", "Fp2", "Fz", "F3", "F4", "F7", "F8", "FC1", "FC2", "FC5",
...
def get_data(data_path,id):
    # da=sio.loadmat(join(data_path,'SHU_Dataset','Subj_'+str(id[0]).zfill(2)+'session_'+str(id[1])+'.mat'))
    da=sio.loadmat(join(data_path,'SHU_Dataset','sub-'+str(id[0]).zfill(3)+'_ses-'+str(id[1]).zfill(2)+'_task_motorimagery_eeg.mat'))
    data=da['data']
